...
import {
  Plus, Search, Edit, Trash2, AlertTriangle, X,
  ImagePlus, Save, Loader2, Eye, EyeOff, Star, Tag, GripVertical,
} from "lucide-react";
...
export default function AdminProductsPage() {
  ...
  const saveOrder = async () => {
    setSavingOrder(true);
    try {
      const items = products.map((p, i) => ({ slug: p.slug, order: i }));
      await dashboardApi.reorderProducts(items);
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          {orderMode ? (
            <>
              <button
                onClick={() => { setOrderMode(false); fetchProducts(); }}
        ...
        <div className="p-4 border-b border-border">
          {orderMode ? (
            <p className="text-sm text-muted-foreground">
              Glissez-déposez les lignes pour réordonner, puis cliquez sur <strong>Sauvegarder l&apos;ordre</strong>.
            </p>
          ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                {orderMode && <th className="w-8 px-2 py-3" />}
                ...
                {!orderMode && <th className="text-left px-4 py-3 text-xs text-muted-foreground font-medium">Actions</th>}
              </tr>
            </thead>
            <tbody>
              {loading
                ...
                : products.map((p, idx) => (
                    <tr
                      ...
                      className={`border-b border-border/50 transition-colors ${orderMode ? "cursor-grab active:cursor-grabbing hover:bg-brand-beige/40" : "hover:bg-muted/20"}`}
                    >
                      {orderMode && (
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() => !orderMode && toggleField(p.slug, "is_featured", p.is_featured)}
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() => !orderMode && toggleField(p.slug, "is_active", p.is_active)}
                          disabled={orderMode}
