...
Efface : commandes, produits, catégories, boutiques, clients, inventaires.
"""
    ...
    def handle(self, *args, **options):
        if not options['confirm']:
            self.stdout.write(self.style.WARNING(
                "\n⚠️  Cette commande supprime TOUTES les données (commandes, produits, boutiques, clients).\n"
        ...
        from apps.products.models import (
            Product, ProductImage, StoreInventory,
            Wishlist, ProductReview, Category,
        )
        ...
        with transaction.atomic():
            ...
            n_inv = StoreInventory.objects.all().delete()[0]
            n_prod = Product.objects.all().delete()[0]
            n_cat = Category.objects.all().delete()[0]

            # Boutiques
            n_stores = Store.objects.all().delete()[0]
        ...
        self.stdout.write(f"   Produits        : {n_prod} ({n_img} images, {n_inv} stocks)")
        self.stdout.write(f"   Catégories      : {n_cat}")
        self.stdout.write(f"   Boutiques       : {n_stores}")
